...
public class OportunidadeController {
	...
	public ResponseEntity<Oportunidade> atualizar(@Valid @RequestBody Oportunidade oportunidade) {
		Optional<Oportunidade> oportunidadeParaAtualizar = oportunidades.findById(oportunidade.getId());
		if(!(oportunidadeParaAtualizar.isPresent())) {
			return ResponseEntity.badRequest().build();
		}
		oportunidadeParaAtualizar.get().setNomeProspecto(oportunidade.getNomeProspecto());
		oportunidadeParaAtualizar.get().setDescricao(oportunidade.getDescricao());
		oportunidadeParaAtualizar.get().setValor(oportunidade.getValor());
		oportunidades.save(oportunidadeParaAtualizar.get());
